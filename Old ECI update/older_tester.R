...
title_ip <- "The Fed's Favorite Metric Declined"
...
title_toplines <- "Across Measures, Nominal Wages Lowered Last Quarter"

#### Graphic Long Term ####
eci_long <- read_csv("data/eci_long.csv") %>% filter(type == "ECI Wage Growth (SIC)") %>% mutate(date = date %m-% months(1))
...
eci_toplines <- eci %>% mutate(value = value) %>% filter(date >= "2017-01-01") %>%
  filter(estimate_code == 2, periodicity_code == "Q", owner_code == 2,
...
incentive_pay %>% select(value, date) %>% mutate(type = "Not Seasonally Adjusted, BLS") %>%
  ...
  scale_color_manual(values=c("#F67280","#FFD3B5")) +
  scale_size_manual(values=c(1.2,1)) +
  scale_alpha_manual(values=c(1,0.25)) +
  geom_text(aes(x=date, y=value, label=num_label2), nudge_y = 0.003, size=3.5, color="#F67280", show.legend = FALSE)
...
eci %>% filter(series_id %in% toplines) %>%
  ...
  mutate(num_label = round(100*value, 1)) %>%
  mutate(num_label2 = ifelse(date >= "2021-09-01", num_label, NA)) %>%
  ggplot(aes(date,value,label=num_label2)) + geom_bar(stat="identity",size=0) + facet_wrap(~typeF) +
  labs(x="", y="", title=title_toplines,
       subtitle="Wages, private workers, excluding incentive paid occupations, 3 month percent change annualized.",
       caption="BLS, ECI, author's calculations, Mike Konczal, Roosevelt Institute") +
  scale_x_date(date_labels = "%b\n%Y", breaks=eci_g1_dates) +
  scale_y_continuous(labels = scales::percent) + theme_lass + theme(legend.position = "none") +
  geom_text(aes(x=date, y=value, label=num_label2), nudge_y = 0.006, size=2, show.legend = FALSE)
...
cpi_dates <- unique(eci$date)
cpi_dates <- sort(cpi_dates, decreasing = TRUE)
cpi_dates = cpi_dates[seq(1, length(cpi_dates), 2)]


incentive_pay_ts %>% select(-type) %>%
  ...
  geom_line(aes(y = wages, color = "ECI Wages")) +
  ...
  labs(subtitle="ECI wages, private workers, excluding incentive paid occupations. Headline CPI. Both 3 month percent change.",
      caption="ECI seasonal adjustment calculated with R's seasonal library, using X-13ARIMA-SEATS, author's calculations, Mike Konczal, Roosevelt Institute",
      title="Soft Landing - LFG!")

ggsave("graphics/eci_difference_incentive_pay.png", width = 12, height=6, dpi="retina")
...
eci %>% filter(series_id == "CIS2020000000000Q") %>%
  ...
  ggplot(aes(x = date)) +
  # One line for Cat rescues
  geom_line(aes(y = cpi, color = "CPI, Headline")) +
  # Another line for Not_Cat rescues
  geom_line(aes(y = wages, color = "ECI, Private Wages")) +
  # stat_difference() from ggh4x package applies the conditional fill
  # based on which of Not_Cat and Cat is larger.
  stat_difference(aes(ymin = wages, ymax = cpi), alpha = 0.25) + theme_lass +
  scale_x_date(date_labels = "%b\n%Y", breaks=cpi_dates) +
  ...
  labs(subtitle="ECI wages and salaries for all private industry workers. Headline CPI. 3-month percent change.",
       caption="Seasonally adjusted. Author's calculations, Mike Konczal, Roosevelt Institute",
       title="Soft Landing - LFG!")

ggsave("graphics/eci_difference.png", width = 12, height=6, dpi="retina")
